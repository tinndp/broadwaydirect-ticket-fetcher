...
# Rowing ListingIdentity (djb2 hash + base-36) and the shared Mongo client live in shared/.
from shared.rowing_mongo import build_listing_id, get_client, get_deterministic_hash_code  # noqa: F401  (re-exported for tests)
...
def write_inventory(mongo_uri: str, mongo_db: str, event: Event, listings: list, price_levels: list) -> int:
    ...
    (see shared.rowing_mongo.get_client) rather than opening and closing a new one per call."""
    from bson.decimal128 import Decimal128
    from decimal import Decimal

    src_event_id = source_event_id(event)
    client = get_client(mongo_uri, mongo_db)
