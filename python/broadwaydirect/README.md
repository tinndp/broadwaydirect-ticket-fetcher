# broadwaydirect (Python)

All commands below assume you're running from the repo's `python/` directory
(`cd python` first if you're at the repo root). See `../../dotnet/BroadwayDirect/README.md`
for the .NET port and `../../discovery/BroadwayDirectShowDiscovery` for the event list.
...
broadwaydirect/
  ...
  tests/
    ...
    Run: python3 -m pytest broadwaydirect -v  (8 tests)
shared/proxy_pool.py - proxy list / {SESSIONID} helpers (moved out of this package, shared with
                       stubhub, paciolanevenue and axs)
```
