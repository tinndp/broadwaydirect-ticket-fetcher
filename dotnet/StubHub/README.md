...
way `dotnet/` (dotnet/BroadwayDirect/README.md) ports `python/broadwaydirect/`: same project
split, added to the same `Crawlers.sln`, reusing `BroadwayDirect.Core` for
...
# or the whole solution (BroadwayDirect + StubHub): dotnet test Crawlers.sln
```
